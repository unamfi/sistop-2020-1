/*Proyecto 2 Simulacion de un Restaurante
...
void entraAlRestaurante(struct cliente* paquito)
{
	...
	printf("\n\n + Llega nuevo cliente a la mesa %d",paquito->numMesa); //Va a la mesa
...
void pideComida(struct cliente *paquito)
{
	...
	if (paquito->numMesa < 3) //Si me sente el las primeras 3 mesas, me corresponde el mesero A
	{
		...
		printf("\n ? La mesa %d llama al mesero A",paquito->numMesa); //Llamo a mi mesero
	...
	}else //De lo contrario, llamo al meseroB
	{
		...
		printf("\n ? La mesa %d llama al mesero B",paquito->numMesa);
...
void saleDelRestaurante(struct cliente * paquito)
{
	...
	printf("\n\n %c Salio cliente de la mesa %d, ya puede entrar otro\n",245,paquito->numMesa);
...
void nvoCliente(void)
{
	...
	printf("\n = La mesa %d esta comiendo",paquito->numMesa);
	Sleep(500); //Como
	...
	printf("\n %c La mesa %d pide su cuenta",33,paquito->numMesa);
...
{										//si uno esta disponible, va y le hace un pedido
	...
	if(flagCocineroA == 'n') //Si el primer cocinero esta desocupado
	{
		...
		flagCocineroA='p';
		printf("\n %c Mesero A encarga pedido %d al cocinero",207,mA->colaPedidos[0]);
	...
	do
	{
		...
		switch(flagMeseroA)
		{
			case 't': //En caso de que haya llamado el cliente tomo su pedido
					{
						...
						{
							printf("\n - mesero A Anoto pedido mesa %d",pedidoA);
			...
			case 'r': //Si llamo el cocineroA, recojo el pedido
						...
						printf("\n E mesero A entrega comida a mesa %d",mA.entrega);
...
{										//si uno esta disponible, va y le hace un pedido
	...
	if(flagCocineroA == 'n') //Si el primer cocinero esta desocupado
	{
		...
		printf("\n %c Mesero B encarga pedido %d al cocinero",207,mB->colaPedidos[0]);
	...
	do
	{
		...
		switch(flagMeseroB)
		{
			case 't': //En caso de que haya llamado el cliente tomo su pedido
					{
						...
						{
							printf("\n - meseroB anoto pedido mesa %d",pedidoB);
			...
			case 'r': //Si llamo el cocineroA, recojo el pedido
						...
						printf("\n E mesero B entrega comida a mesa %d",mB.entrega);
						cocineroA.unlock();//Desocupo a mi cocinero
	...
	do
	{
		...
		switch(flagCocineroA)
		{
			case 'p'://Si le hizo un pedido meseroA
						...
						Sleep(500);//En lo que preparo los alimentos
						
						printf("\n %c Cocina ya  preparo pedido mesa %d",244,structCocinero.pedido);
...
{	
	...
	printf("\n\n Listo, presiona ENTER para terminar el programa");
	fflush(stdin);
	getchar();
