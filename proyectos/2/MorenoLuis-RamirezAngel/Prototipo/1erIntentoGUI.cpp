...
std::condition_variable esperaUnCambio; //Para detener o activar al hilo que dibuja la pantalla
std::mutex mEsperaUnCambio;

std::condition_variable esperaTuDibujo; //Para evitar que el hilo que hizo el cambio, avance


struct cliente
{
	short int numMesa; //En que mesa se sienta
	short int x; //coordenadas
	short int y;
//	char siendoAnimado = 'n';
};
...
//	printf("\n\tEstoy en mesa %d",paquito->numMesa); //Va a la mesa
	...
	//Dibujo a mi persona dentro del restaurante
	//	std::unique_lock<std::mutex> esperaDib(mEsperaTuDibujo);
	//	while(flagDibujaPantalla != 'n')	//Mientras mi pantalla se dibuje
	//	{
	//		esperaTuDibujo.wait(esperaDib);	//espera antes de solicitarle otro cambio
	//	}
		
		flagDibujaPantalla ='s'; //Dile a la pantalla que lo dibuje
		esperaUnCambio.notify_all();
		
		std::unique_lock<std::mutex> esperaD(mEsperaTuDibujo);
		while(flagDibujaPantalla != 'n')	//Mientras mi pantalla se dibuje
		{
			esperaTuDibujo.wait(esperaD);	//espera antes de continuar
		}
...
void nvoCliente(void)
{
	...
	std::mutex esperaComida; //Creo mutex local para quedarme en la mesa esperando mi comida
	std::mutex mEsperaTuDibujo;
...
//	Sleep(6000);
	...
	//Dibujo a mi persona frente al cajero
	//	std::unique_lock<std::mutex> espera(mEsperaTuDibujo);
	//	while(flagDibujaPantalla != 'n')	//Mientras mi pantalla se dibuje
	//	{
	//		esperaTuDibujo.wait(espera);	//espera antes de solicitarle otro cambio
	//	}
...
void funcionMadre (void)
{
	...
	//	}
		...
		tiempo = rand() %1500;
		Sleep(500+tiempo); //Espero antes de crear un nuevo hilo
...
void dibujaPantalla(void)
{
	do
	{
		std::unique_lock<std::mutex> espera(mEsperaUnCambio);
		while( flagDibujaPantalla == 'n' )	//Mientras no haya ocurrido un cambio
		{
			esperaUnCambio.wait(espera);	//esperamos
		}
		
		system("cls");
		limpia(pantalla);
		dibujaRestaurante(pantalla,comidaEnMesas);
		dibujaFila(pantalla,genteFormada);
		dibujaGenteMesas(pantalla,mesas);
		dibujaGenteCaja(pantalla,flagCajero);
		
		imprimePantalla(pantalla);
		
		flagDibujaPantalla = 'n'; //ya acabe de dibujar
		esperaTuDibujo.notify_all(); //Di a todos los que te esperan que pueden continuar
		
	//	sleep(200); //Espera un poco antes de mostrarte como desocupado =)
