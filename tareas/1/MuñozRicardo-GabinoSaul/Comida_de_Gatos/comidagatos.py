...
class plato:
    def __init__(self):
        ...
        print ("Sirviendo comida en el plato  %1.3f" % self.plato)
...
class cats:
    def __init__(self):
        ...
        print ("Ha llegado el gato numero %1.3f" % self.cats)
...
def pongo_plato():
    ...
    while True:
        ...
        if comiendo == MaxEat:
            mutex2.release()
            if(acabado != 0):
                print ("Alguien ha comido del plato (%1.3f)" % acabado)
            EComiendo.acquire()
        else:
            mutex2.release()
            print ("Esta libre el plato (%1.3f)" % numero)
            comiendo += 1
